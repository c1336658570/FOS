...
// __attribute__ ((__section__ (".data.init"))); pte_t pte0[1024]
// __attribute__((__aligned__(PAGE_SIZE))) __attribute__ ((__section__
// (".data.init")));   // 0 - 4M pte_t pte1[1024]
// __attribute__((__aligned__(PAGE_SIZE))) __attribute__ ((__section__
// (".data.init")));   // 4 - 8M

pgd_t pgd[1024] __attribute__((__aligned__(PAGE_SIZE)))
...
/* 创建gdt描述符 */
static struct gdt_desc __init make_gdt_desc(uint32_t *desc_addr, uint32_t limit,
                                            uint8_t attr_low,
                                            uint8_t attr_high) {
...
static void __init page_create(void) /* reate page*/
{
  ...
  // set  gs
  *((struct gdt_desc *)0x918) = make_gdt_desc(
      (uint32_t *)0xc00b8000, 0x00007, GDT_DATA_ATTR_LOW_DPL0, GDT_ATTR_HIGH);

  // set gdt
  uint64_t gdt_operand =
      ((8 * 6 - 1) | ((uint64_t)(uint32_t)0xc0000900 << 16));  // 6个描述符大小
  ...
  asm volatile("call %0" : : "m"(flush));  // 刷新流水线
  return;
}
...
//     swapper_pg_dir[0].pgd = (uint32_t)pte0 | PAGE_PRESENT | PAGE_WRITE |
//     PAGE_USER; swapper_pg_dir[1].pgd = (uint32_t)pte1 | PAGE_PRESENT |
//     PAGE_WRITE | PAGE_USER; swapper_pg_dir[768].pgd = (uint32_t)pte0 |
//     PAGE_PRESENT | PAGE_WRITE | PAGE_USER; swapper_pg_dir[769].pgd =
//     (uint32_t)pte1 | PAGE_PRESENT | PAGE_WRITE | PAGE_USER;
//     // pgd[1023] = (uint32_t)pgd | PAGE_PRESENT | PAGE_WRITE | PAGE_USER;
...
//     //set  gs
//     *((struct gdt_desc*)0x918) = make_gdt_desc((uint32_t*)0xc00b8000,
//     0x00007, GDT_DATA_ATTR_LOW_DPL0, GDT_ATTR_HIGH);

//     //set gdt
//     uint64_t gdt_operand = ((8 * 6 - 1) | ((uint64_t)(uint32_t)0xc0000900 <<
//     16));   // 6个描述符大小
...
static void __init gdt_create(void) {
  ...
  /* gdt段基址为0x900,把tss放到第4个位置,也就是0x900+0x20的位置 900 908 910 918
   * 920 928 930 */
  *((struct gdt_desc *)0x900) = make_gdt_desc((uint32_t *)0, 0, 0, 0);
  *((struct gdt_desc *)0x908) = make_gdt_desc(
      (uint32_t *)0, 0xfffff, GDT_CODE_ATTR_LOW_DPL0, GDT_ATTR_HIGH);
  *((struct gdt_desc *)0x910) = make_gdt_desc(
      (uint32_t *)0, 0xfffff, GDT_DATA_ATTR_LOW_DPL0, GDT_ATTR_HIGH);
  *((struct gdt_desc *)0x918) = make_gdt_desc(
      (uint32_t *)0xb8000, 0x00007, GDT_DATA_ATTR_LOW_DPL0, GDT_ATTR_HIGH);
  /* 在gdt中添加dpl为3的数据段和代码段描述符 */
  *((struct gdt_desc *)0x920) = make_gdt_desc(
      (uint32_t *)0, 0xfffff, GDT_CODE_ATTR_LOW_DPL3, GDT_ATTR_HIGH);
  *((struct gdt_desc *)0x928) = make_gdt_desc(
      (uint32_t *)0, 0xfffff, GDT_DATA_ATTR_LOW_DPL3, GDT_ATTR_HIGH);

  /* gdt 16位的limit 32位的段基址 */
  uint64_t gdt_operand =
      ((8 * 6 - 1) | ((uint64_t)(uint32_t)0x900 << 16));  // 7个描述符大小
  asm volatile("lgdt %0" : : "m"(gdt_operand));
  asm volatile("call %0" : : "m"(flush));  // 刷新流水线
