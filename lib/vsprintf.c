 ...
 * @base: 要解析的数字的基数(例如，10代表十进制)。如果为0，则根据字符串的前缀决定基数。
 *
...
#define LARGE	64		/* use 'ABCDEF' instead of 'abcdef' */		// 使用大写字母'ABCDEF'而不是小写字母'abcdef' 
...
static char * number(char * str, long long num, int base, int size, int precision, int type)
{
	...
	if (base < 2 || base > 36)					// 如果进制不在2到36之间，则返回错误
		return 0;
	c = (type & ZEROPAD) ? '0' : ' ';		// 如果使用零填充，则填充字符为'0'，否则为' '
	sign = 0;			// 符号位初始化为0
	if (type & SIGN) {
		if (num < 0) {
			sign = '-';
	...
	if (num == 0)						// 如果数值为0，则直接将'0'存入临时字符数组
		tmp[i++]='0';
	else while (num != 0)		// 将数值按给定进制转换为字符，并存入临时字符数组
		tmp[i++] = digits[do_div(num,base)];
	if (i > precision)			// 如果实际转换得到的字符数大于指定的精度，则将精度设置为实际字符数
		precision = i;
	size -= precision;			// 计算剩余可用空间
	if (!(type&(ZEROPAD+LEFT)))		// 如果不是左对齐且不使用零填充，则在剩余空间前填充空格
		while(size-->0)
			*str++ = ' ';
	if (sign)
		*str++ = sign;	// 存储符号位
...
int vsprintf(char *buf, const char *fmt, va_list args)
{
	...
	int precision;		/* min. # of digits for integers; max			// 整数的最小位数；字符串的最大字符数
				   number of chars for from string */
	int qualifier;		/* 'h', 'l', or 'L' for integer fields */	// 用于整数字段的限定符（'h'、'l'、'L'、'Z'）	short，long，long double，size_t
	...
	for (str=buf ; *fmt ; ++fmt) {
		...
		else if (*fmt == '*') {							// 宽度在format字符串中未指定，但是会作为附加整数值参数放置于要被格式化的参数之前
			++fmt;
		...
		if (*fmt == '.') {
			...
			else if (*fmt == '*') {						// 精度在format字符串中未指定，但是会作为附加整数值参数放置于要被格式化的参数之前
				++fmt;
				/* it's the next argument */
		...
		case 'p':			// 指针
			...
			str = number(str,
				...
				field_width, precision, flags);		// 将指针转换为字符串
			continue;
		...
		default:		// 未知格式化字符
			*str++ = '%';				// 遇到未知的格式化字符，则将%字符和后面的字符原样复制到目标缓冲区中
			if (*fmt)
				*str++ = *fmt;
			else
				--fmt;
